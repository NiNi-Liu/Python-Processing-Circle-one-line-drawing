...
int[] thred = {180,160};//Change the threshold

void setup() {
  size(1920, 1080);
  sunflower = loadImage("yendog.jpg");//Put the picture HERE!!
  //image(sunflower, 0, 0);
  bk_list = new IntList();
  gr_list = new IntList();
  wt_list = new IntList();
  ...
  for (int i =380; i<width-720; i+= spacing) {
    for (int j = 185; j<height-80; j+= spacing) {
      a += 1;
      int loc = i+j*width;
